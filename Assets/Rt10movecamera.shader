﻿Shader "Unlit/SingleColor"
{
	Properties
	{
	_floatxaxis("Move sphere", Range(-10,10)) = 0
	_refractionIndex("Refraction Index", Range(0,3)) = 1.5
	_maxbounces("max bounses", Range(1,100)) = 10
	_raysprpixel("Rays per pixel", Range(1,1000)) = 1
	_fov("Field of View", Range(1,180)) = 90
	_camerapos("Camera position", Vector) = (-2,2,1)
	_cameralookatpos("Camera Look-at position", Vector) = (0,0,-1)
	}
	...
	{
		if (i == 0) { sph.center = vec3( 0, 0, -1 + _floatxaxis); sph.radius = 0.5; sph.materialtype = 0; sph.matproperties.xyz = vec3(0.1, 0.2, 0.5); sph.fuzz = 1.5;}
	...
	class camera 
	{
		ray get_ray(ray r, float u, float v) 
		{
		...
		void make() 
		{
			...
			float theta = _fov * 3.14159265 / 180.0;
			float half_height = tan(theta / 2.0);
			float half_width = 2.0 * half_height;
			
			vec3 w = normalize(_camerapos - _cameralookatpos);
			vec3 u = normalize(cross(vec3(0, 1, 0), w));	
			vec3 v = cross(w, u);
			
			lower_left_corner = _camerapos - half_width * u - half_height * v - w;
	...
	fixed4 frag(v2f i) : SV_Target 
	{
		...
		{
			ray r;
			r = c.get_ray(r, x + 0.003 * rand(), y + 0.003 * rand());
			col += ray_color(r);
		}
		col /= _raysprpixel;
